...
const meta = {
  ...
  // No 'autodocs': this is an internal isolation proof/regression story, not
  // consumer API documentation. Its autogenerated Docs page rendered the
  // popover clipped — Storybook's Docs preview wraps each story in a
  // `transform: scale()` zoom container, which establishes a containing block
  // that traps Radix's `position: fixed` popover (portaled into the shadow
  // root here) inside the small preview box. Canvas view is unaffected, as is
  // real consumer usage (no such transform ancestor). The `integration`-tagged
  // stories below still run as interaction tests.
  args: {
    versionId: 111,
  },
